...
pub trait ClientContext: Send + Sync {
    /// Whether to install [`ClientContext::refresh_oauth_token`] as the
    /// SASL `OAUTHBEARER` token refresh callback.
    ///
    ...
    /// mechanism.
    const ENABLE_REFRESH_OAUTH_TOKEN_CALLBACK: bool = false;
    ...
    /// Refreshes an OAuth token from the provided configuration.
    ///
    ...
    /// also set [`ClientContext::ENABLE_REFRESH_OAUTH_TOKEN_CALLBACK`] to true.
    ///
...
impl<C: ClientContext> Client<C> {
    ...
    pub fn new(
        ...
        if C::ENABLE_REFRESH_OAUTH_TOKEN_CALLBACK {
            unsafe {
...
/// returned from [`ClientContext::refresh_oauth_token`]. The token and
...
pub(crate) unsafe extern "C" fn native_oauth_refresh_cb<C: ClientContext>(
    ...
    let res: Result<_, Box<dyn Error>> = (|| {
        ...
        let token_info = context.refresh_oauth_token(oauthbearer_config.as_deref())?;
